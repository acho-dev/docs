...
The Pricing Simulation allows you to build a pricing model to convert usage records to a bill. You can interact with the UI to select the telemetry source (session/trace/span/raw) and the multiplier to see the predicted earning and margin from the pricing model.

# Pricing Simulation SDK
